...
const Home = ({
  ...
  const refreshApplications = async () => {
    await fetchApplications().then((data) => setApplications(data));
  };
  ...
  const handleUpdate = async (e: FormEvent<HTMLFormElement>) => {
    ...
    await updateApplication({ id, body }).then(() => refreshApplications());
  ...
  const handleDelete = async (id: string) => {
    ...
    await deleteApplication({ id }).then(() => refreshApplications());
  ...
  const handleAutoCollect = async (id: string) => {
    ...
    await getListingData({
      ...
      .catch((e) => addToast({ color: "danger", title: e }));
  };
  ...
  const renderWelcome = () => {
    const quickAdd = async (e: FormEvent<HTMLFormElement>) => {
      ...
      await scrapeResponse({ url, apiKey: firecrawlKey! })
        .then((res) => res)
        .then(async (data) => {
          const scrapedData = data;
