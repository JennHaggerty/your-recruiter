...
export const automatedCoverLetter = async (args: {
  job: Job;
  openAiKey: string;
}) => {
  ...
  try {
    ...
    const resumeWording = resume ? ` and my resume ${resume}` : ``;
    const options = `
    ...
    const prompt =
      `Based on this job description ${posting} ${resumeWording} ${coverLetterWording} ${options} with this date ` +
      new Date();
