...
export const scrapeResponse = async(args: { url: string, apiKey: string }) => {
  ...
  try {
    const scrapeResult = await app.extract([url], {
      schema: schema,
      prompt: 'Grab the markdown and url. What is the job role? Determine if the position is in office, remote, or hybrid. What is the pay? What are the tools? What does it say about a cover letter?s',
    });
